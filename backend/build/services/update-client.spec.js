...
// src/services/update-client.spec.ts
...
describe("Update Client Use Case", () => {
  ...
  it("should update a client that already exists in the database", async () => {
    await clientRepository.create({
      ...
      dataFundation: /* @__PURE__ */ new Date(),
    ...
    globalExpect(clientUpdated.tradeName).toBe("New Trade Name");
    globalExpect(clientUpdated.locationAddress).toBe("New Address");
    globalExpect(clientUpdated.id).toBe("client-1");
    globalExpect(clientUpdated.updatedAt).toEqual(globalExpect.any(Date));
  });
